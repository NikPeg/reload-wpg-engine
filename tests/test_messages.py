...
async def test_game(game_engine: GameEngine):
    ...
    return await game_engine.create_game(
        name="Test Game", description="Test game for message system", setting="Modern", max_players=5
    )
...
async def regular_player(game_engine: GameEngine, test_game):
    ...
    country = await game_engine.create_country(
        game_id=test_game.id, name="Test Kingdom", description="A test kingdom", capital="Test City", population=100000
    )
...
class TestMessageSystem:
    ...
    async def test_create_admin_reply(self, game_engine: GameEngine, regular_player: Player, test_game):
        ...
        original_message = await game_engine.create_message(
            player_id=regular_player.id, game_id=test_game.id, content="Player question", is_admin_reply=False
        )
