...
class Message(Base):
    ...
    reply_to: Mapped[Optional["Message"]] = relationship("Message", remote_side="Message.id", back_populates="replies")
    replies: Mapped[list["Message"]] = relationship("Message", back_populates="reply_to")

    def __repr__(self) -> str:
        return f"<Message(id={self.id}, player_id={self.player_id}, " f"is_admin_reply={self.is_admin_reply})>"
