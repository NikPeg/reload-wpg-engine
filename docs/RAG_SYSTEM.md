...
- **Длинные сообщения** → автоматическая разбивка на части (лимит Telegram 4096 символов)
- **Ошибки логируются** в консоль для отладки

### Разбивка длинных сообщений

Система автоматически обрабатывает длинные RAG-справки:

1. **Проверка длины**: Если сообщение больше 4096 символов
2. **Умная разбивка**:
   - Сначала по абзацам (двойные переносы строк)
   - Затем по предложениям (если абзац слишком длинный)
3. **Сохранение форматирования**: Каждая часть форматируется отдельно
4. **Reply-to**: Только первая часть отправляется ответом на исходное сообщение
5. **Последовательная отправка**: Части отправляются одна за другой

**Пример разбивки:**
```
Часть 1 (ответ на сообщение игрока):
📊 **RAG-справка**
**Запрос игрока:** ...
### **1. Военная мощь:**
...
